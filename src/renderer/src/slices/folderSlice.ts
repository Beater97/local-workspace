...
export const createFolder = createAsyncThunk('folder/create', async (newFolder: Partial<Folder>, { dispatch }) => {
  const res = await window.api.folder.create(newFolder);
  if (!res.success) console.log(res);
  dispatch(fetchFolders());
  return res.data;
});
export const updateFolder = createAsyncThunk('folder/update', async ({ id, data }: { id: number; data: Partial<Folder> }, { dispatch }) => {
...
const folderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchFolders.pending, (state) => { state.loading = true; })
      ...
      .addCase(createFolder.pending, (state) => { state.loading = true; })
      .addCase(createFolder.fulfilled, (state) => { state.loading = false; })
      ...
      .addCase(updateFolder.fulfilled, (state) => { state.loading = false; })
      ...
      .addCase(deleteFolder.fulfilled, (state) => { state.loading = false; })
