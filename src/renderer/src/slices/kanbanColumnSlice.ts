...
export const createKanbanColumn = createAsyncThunk('kanbanColumn/create', async (newColumn: Partial<KanbanColumn>, { dispatch }) => {
  const res = await window.api.kanbanColumn.create(newColumn);
  if (!res.success) console.log(res);
  dispatch(fetchKanbanColumns());
  return res.data;
});
export const updateKanbanColumn = createAsyncThunk('kanbanColumn/update', async ({ id, data }: { id: number; data: Partial<KanbanColumn> }, { dispatch }) => {
  const res = await window.api.kanbanColumn.update(id, data);
  if (!res.success) console.log(res);
  dispatch(fetchKanbanColumns());
  return;
});
export const deleteKanbanColumn = createAsyncThunk('kanbanColumn/delete', async (id: number, { dispatch }) => {
...
const kanbanColumnSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchKanbanColumns.pending, (state) => { state.loading = true; })
      ...
      .addCase(createKanbanColumn.pending, (state) => { state.loading = true; })
      .addCase(createKanbanColumn.fulfilled, (state) => { state.loading = false; })
      .addCase(createKanbanColumn.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Errore durante la creazione';
      })

      .addCase(updateKanbanColumn.pending, (state) => { state.loading = true; })
      .addCase(updateKanbanColumn.fulfilled, (state) => { state.loading = false; })
      .addCase(updateKanbanColumn.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Errore durante l\'update';
      })

      .addCase(deleteKanbanColumn.pending, (state) => { state.loading = true; })
      .addCase(deleteKanbanColumn.fulfilled, (state) => { state.loading = false; })
