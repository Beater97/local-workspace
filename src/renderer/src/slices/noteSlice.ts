...
export const createNote = createAsyncThunk('note/create', async (newNote: Partial<Note>, { dispatch }) => {
  const res = await window.api.note.create(newNote);
  if (!res.success) console.log(res);
  dispatch(fetchNotes());
  return res.data;
});
export const updateNote = createAsyncThunk('note/update', async ({ id, data }: { id: number; data: Partial<Note> }, { dispatch }) => {
...
const noteSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteNote.fulfilled, (state) => { state.loading = false; })
