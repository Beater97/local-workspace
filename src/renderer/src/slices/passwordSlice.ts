...
export const createPassword = createAsyncThunk('password/create', async (newPassword: Partial<Password>, { dispatch }) => {
  const res = await window.api.password.create(newPassword);
  if (!res.success) console.log(res);
  dispatch(fetchPasswords());
  return res.data;
});
export const updatePassword = createAsyncThunk('password/update', async ({ id, data }: { id: number; data: Partial<Password> }, { dispatch }) => {
  const res = await window.api.password.update(id, data);
  if (!res.success) console.log(res);
  dispatch(fetchPasswords());
  return;
});
export const deletePassword = createAsyncThunk('password/delete', async (id: number, { dispatch }) => {
...
const passwordSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchPasswords.pending, (state) => { state.loading = true; })
      ...
      .addCase(createPassword.pending, (state) => { state.loading = true; })
      .addCase(createPassword.fulfilled, (state) => { state.loading = false; })
      ...
      .addCase(deletePassword.pending, (state) => { state.loading = true; })
      .addCase(deletePassword.fulfilled, (state) => { state.loading = false; })
